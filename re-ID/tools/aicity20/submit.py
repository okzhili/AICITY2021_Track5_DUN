...
def write_result_with_track(indices, dst_dir, tracks, name,topk=100):
    indices = indices[:, :topk]
    if not os.path.exists(dst_dir):
        os.makedirs(dst_dir)
    m, n = indices.shape
    ...
    with open(os.path.join(dst_dir, name+'.txt'), 'w') as f:
        for i in range(m):
            out_list=[]
            for rank in reranked_results[i]:
                out_list.append(int(rank.split('.')[0]))
            write_line = ' '.join(map(str, out_list)) + '\n'
            f.write(write_line)
#
# '''
